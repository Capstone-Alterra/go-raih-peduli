...
func (_m *Repository) HistoryVolunteerVacanciesRegisterByUser(userID int) ([]history.VolunteerRegistered, error) {
	ret := _m.Called(userID)

	var r0 []history.VolunteerRegistered
	var r1 error
	if rf, ok := ret.Get(0).(func(int) ([]history.VolunteerRegistered, error)); ok {
		return rf(userID)
	}
	if rf, ok := ret.Get(0).(func(int) []history.VolunteerRegistered); ok {
		r0 = rf(userID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]history.VolunteerRegistered)
		}
	}
