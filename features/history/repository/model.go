...
func (mdl *model) HistoryUserTransaction(userID int) ([]history.Transaction, error) {
	...
	if err := mdl.db.Preload("User").Table("transactions").Joins("JOIN users ON transactions.user_id = users.id").Joins("JOIN fundraise ON transactions.fundraise_id = fundraises.id").Where("transactions.user_id = ?", userID).Find(&transaction).Error; err != nil {
		return nil, err
	}
