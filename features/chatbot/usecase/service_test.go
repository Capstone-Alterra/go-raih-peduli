...
func TestSetReplyMessage(t *testing.T) {
	...
	var questionTime = time.Date(2023, time.April, 19, 15, 30, 0, 0, time.UTC)
	var replyTime = time.Date(2023, time.April, 19, 15, 50, 0, 0, time.UTC)
	...
	t.Run("Success", func(t *testing.T) {
		...
		repository.On("GetTimeNow").Return(questionTime).Once()
		openai.On("GetAppInformation", input.Message, data).Return("this answer generated by gpt", nil).Once()
		repository.On("GetTimeNow").Return(replyTime).Once()
